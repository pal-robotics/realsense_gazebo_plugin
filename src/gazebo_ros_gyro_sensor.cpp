...
GZ_REGISTER_SENSOR_PLUGIN(gazebo::GazeboRosgyroSensor)

gazebo::GazeboRosgyroSensor::GazeboRosgyroSensor(): SensorPlugin()
{
...
void gazebo::GazeboRosgyroSensor::Load(gazebo::sensors::SensorPtr sensor_, sdf::ElementPtr sdf_)
{
  sdf=sdf_;
  ...
  connection = gazebo::event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboRosgyroSensor::UpdateChild, this, _1));

  last_time = sensor->LastUpdateTime();
}

void gazebo::GazeboRosgyroSensor::UpdateChild(const gazebo::common::UpdateInfo &/*_info*/)
{
  ...
  if(imu_data_publisher.getNumSubscribers() > 0)
  {
    ...
    imu_msg.linear_acceleration.x = 0; //accelerometer_data.X() + GuassianKernel(0,gaussian_noise);
    imu_msg.linear_acceleration.y = 0; //accelerometer_data.Y() + GuassianKernel(0,gaussian_noise);
    imu_msg.linear_acceleration.z = 0; //accelerometer_data.Z() + GuassianKernel(0,gaussian_noise);
...
double gazebo::GazeboRosgyroSensor::GuassianKernel(double mu, double sigma)
{
...
bool gazebo::GazeboRosgyroSensor::LoadParameters()
{
...
gazebo::GazeboRosgyroSensor::~GazeboRosgyroSensor()
{
  if (connection.get())
  {
    connection.reset();
